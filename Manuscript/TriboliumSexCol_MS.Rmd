...
We tracked a total of `r nrow(dd)` *T. castaneum* populations in a novel thermal environmental until extinction, or for up to `r ngens` generations. In the first generation, `r num2word(nrow(filter(dd,Offspring_Gen1 == 0, Treatment == "Monogamous")))` populations founded by singly-mated females went extinct, while no populations founded by doubly-mated females went extinct. By generation 10, `r num2word(nrow(filter(dd,Offspring_Gen10 == 0, Treatment == "Monogamous")))` monogamous populations and `r num2word(nrow(filter(dd,Offspring_Gen10 == 0, Treatment == "Polyandrous")))` polyandrous populations were extinct (Fig. 1A). The effect of treatment on time to extinction was significant (P = `r round3(summary(coxph(Surv(time = dd$SurvTime,event = dd$Cens)~Treatment,data=dd))$coefficients[5])`).
...
FigB <- ddlong %>%
  ...
  scale_colour_manual(values = mycols)+
  geom_smooth(aes(x = as.numeric(Generation)))+
